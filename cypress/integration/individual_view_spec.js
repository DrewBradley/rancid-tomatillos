context('Individual', () => {
  ...
  describe('Individual Movie Elements', () => {

    it('Should be able to navigate to individual movie and render footer elements', () => {
      cy.visit('http://localhost:3000/movie/581392')
        cy.get('footer', { timeout: 10000 }).should('be.visible')
        cy.get('footer').find('img').should('have.attr', 'src', "https://image.tmdb.org/t/p/original//sy6DvAu72kjoseZEjocnm2ZZ09i.jpg")
        cy.get('button').click()
    ...
    it('Should be able to navigate between main and individual movie views with back button', () => {
      cy.visit('http://localhost:3000/')
